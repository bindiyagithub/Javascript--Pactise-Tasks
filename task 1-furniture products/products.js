const products = [
   ...
   function filterProduct()
   {
    let filterData =  products.filter(function(value) 
    {
      ...
      for(let d in value)
     {
         ...
         if(value[d].toString().indexOf('sofa') >=0)
         {
             return true;
         }
   ...
   document.getElementById("searchTerm").innerHTML = filterData.join(""); 
   ...
   function display()
  {
    ...
    const mynewProduct = products.map(function(value){
      ...
      return `<article class="product" id = "products">
        <img
         src="${value.image}"
    ...
    //now display companies 

       const mynewCompany = products.map((value)=>
        {
            return `<article class="companies" id="companies">
            <button class="company-btn" 
            onclick = "filterCompany('${value.company}')">${value.company}</button>
            </article>`;
        });

   // document.getElementById("companies").innerHTML = mynewCompany.join(" ");
 
   // now remove duplicate company from company name by using reduce() 
  
    let reduceCompany = mynewCompany.reduce((prevalue , value , Index , array)=>{
  
            if(prevalue.includes(value) == false)
            {
               prevalue.push(value);
            }
            return prevalue;
    ...
    //console.log(reduceCompany);
    document.getElementById("companies").innerHTML = reduceCompany.join(" ");
  
 }     
 display();


///////////// filter the products by clicking on company- buttons /////////
   
      function filterCompany(compName){
        let filterCompanyProducts = products.filter((value)=>{
              return  value.company.toLowerCase() == compName.toLowerCase();
        });

        display(filterCompanyProducts);
        console.log(filterCompanyProducts);
      }
      

    
  
/////////////  arrange the price into ASC & DESC order   /////////////

    function arrangeData()
    {
      let displayPrice = document.getElementById("priceSorting").value;
       products.sort((a,b) =>
      {

          // here we use switch case instead of if.....else...if condition
            switch(displayPrice)
            {
                case "Price in Asc":
